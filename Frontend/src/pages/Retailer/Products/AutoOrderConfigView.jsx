...
const AutoOrderConfigView = ({product_id}) => {
            ...
            if (data.hasOwnProperty("message")){
                ...
                if(error_length > 0 && error_length < 4){
                    if(data.error.hasOwnProperty("qunt_reach_level"))
                        CustomErrorToast({ msg: "quantity reach level: " + data.error['qunt_reach_level'], dur: 3000 });
                    else if(data.error.hasOwnProperty("ordering_amount"))
                        CustomErrorToast({ msg: "ordering amount: Ensure this value is greater than 0, and integer value.", dur: 3000 });
                    else if(data.error.hasOwnProperty("confirmation_status"))
                        CustomErrorToast({ msg: "confirmation status: " + data.error['confirmation_status'], dur: 3000 });
                    }
                else{
                    CustomErrorToast({ msg: data.error, dur: 3000 });
                }
    ...
    const deleteConfig = async () => {
        try {
            ...
            const { data } = await ax.delete(`${API}config/auto-order-config/delete-product-config/`, {data:{product_id:product_id}});
            console.log(data)
            if (data.hasOwnProperty("message")){
                CustomSuccessToast({ msg: data.message, dur: 3000 });
                setConfigData({
        ...
        <Card className="py-5 space-y-3">
            <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
                ...
                <p className="text-default-500">
                    Tweak your product Auto-order configuration: quantity reach levels, ordering amount, and whether confirmation is required or not.
                </p>
            </CardHeader>
